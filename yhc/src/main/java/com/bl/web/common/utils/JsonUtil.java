...
import com.bl.web.common.annotation.JsonFieldAnnotation;
...
public class JsonUtil {
	...
	public static String beanToJson(Object obj) {
		...
		try {
			...
			ArrayList<Boolean> fnAnList = new ArrayList<Boolean>();
			for(int i=0; i<fields.length;i++ ){
				fnList.add(fields[i].getName());
				fnLowList.add(fields[i].getName().toLowerCase());
				if(fields[i].isAnnotationPresent(JsonFieldAnnotation.class)){
					if(fields[i].getAnnotation(JsonFieldAnnotation.class).nullDisp()){
						fnAnList.add(true);
					} else {
						fnAnList.add(false);
					}
			...
			for (int i = 0; i < voPds.length; i++) {
				...
				if (object != null && !object.getClass().getName().equals("java.lang.Class")) {
					...
					if(fnAnList.contains(propName.toLowerCase()) && fnAnList.get(fnLowList.indexOf(propName.toLowerCase()))){
						sb.append('\"');
						sb.append(propName);
						sb.append('\"');
						sb.append(':');
	...
	public static void main(String[] args) {
		System.out.println(JsonUtil.string2Json("{}"));
		try {
