package com.bl.web.common.config;
...
public class PropertyConfig {
	...
	public void load() {
		...
		if (stream == null) {
			...
			try {
				props.load(stream);
				log.debug("loaded properties from resource " + configFileName +
						": " +
						props);
			} catch (Exception e) {
	...
	public boolean getBoolean(String key, boolean defaultValue) {
		...
		for (int i = 0; i < PropertyConfig.TRUE_VALUES.length; i++) {
			if (s.equalsIgnoreCase(PropertyConfig.TRUE_VALUES[i])) return true;
		}
		for (int i = 0; i < PropertyConfig.FALSE_VALUES.length; i++) {
			if (s.equalsIgnoreCase(PropertyConfig.FALSE_VALUES[i])) return false;
		}
		return defaultValue;
	}
